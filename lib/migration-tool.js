...
AbstractMemberTransformation.prototype.detect = function (node, parent, stack) {
  ...
  if (node.type === Syntax.Property) {
    ...
    if (parentType) {
      return this.onDetectProperty(keyName, parentType, node, parent);
    } else {
      return null;
    }
  } else {
...
TypeRemovedTransformation.prototype.perform = function (node, parent) {};
...
ChangeNamespaceTransformation.prototype.perform = function (node, parent) {
  var transformationTargets = this.transformationTargets;
...
ChangeReturnTypeTransformation.prototype.onDetectProperty = ChangeReturnTypeTransformation.prototype.onDetectMember;
 ...
 */
AdjustCommentsTransformation.prototype.perform = function perform(node, parent) {
 ...
 * @extends AbstractTransformation
 */
function NamedConstructorsTransformation() {
  AbstractTransformation.call(this, null);
}
...
NamedConstructorsTransformation.prototype.perform = function (node, parent) {
...
ConfidentCommentResolver.prototype.resolve = function (node, parent) {
  ...
  if (this.keepOldLine) {
    ...
    rawComments.push(getLineForNode(node));
  }
...
ImpossibleCommentResolver.prototype.resolve = function (node, parent) {
  node = AbstractResolver.prototype.resolve.call(this, node, parent);
  commentsCount++;
...
module.exports = function (files, finishedCallback) {
  ...
  var transformations = [
    ...
    new NamedConstructorsTransformation(),
  ...
  async.map(files, function (file, callback) {
    ...
    async.each(files, function (file, callback) {
      var code = Transformer.transformFile(file.src, file.code);
