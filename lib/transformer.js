...
/**
 * esprima needs to know whether the code represents a script or a module.
 * We just try script, and resort to module if that fails (usually because of "unexpected token" for
 * e.g. import/export statements).
 * @see https://github.com/jquery/esprima/blob/master/docs/syntactic-analysis.md#distinguishing-a-script-and-a-module
 * @param {string} code - the code to be parsed
 * @returns The parsed AST
 */
function parse(code) {
  try {
    var ast = esprima.parseScript(code, parserOptions);
    return ast;
  } catch (e) {
    var ast = esprima.parseModule(code, parserOptions);
    return ast;
  }
}
...
var generatorOptions = {
  ...
  comment: true,
  parse: parse // we need this to prevail the correct "raw" literals (e.g. correct quotes)
};
...
var ternServer = new tern.Server({
  async: false,
  defs: [
    require("./yfilesDefs.js"),
    require("./browserDefs.js"),
    require("./ecmascriptDefs.js")
  ]
});
 ...
 * @param {Node[]} stack
 */
AbstractTransformation.prototype.perform = function perform(node, parent, stack) {
  throw "not implemented"
};
